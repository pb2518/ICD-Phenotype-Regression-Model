...
genotypes=as.data.frame(e[,c(1,2)],stringsAsFactors=FALSE) #column 2 for BAG3, column 3 for FHOD3
...
results=phewas_ext(data,phenotypes=names(phenotypes),genotypes=c("BAG3"),covariates=c("sex"), cores=1,min.records=20)

#output data table of results
write.table(results,"/rdsgpfs/general/project/lms-ware-analysis/live/xiao/M.sc/ICD_result_BAG3.txt",row.names=FALSE,quote=FALSE,col.names=TRUE,sep="\t") 
results<-read.table("/rdsgpfs/general/project/lms-ware-analysis/live/xiao/M.sc/ICD_result_BAG3.txt",header=TRUE,sep="\t") #read in data summary file


#output plot of results
png("/rdsgpfs/general/project/lms-ware-analysis/live/Prarthna/ICD_result_BAG3plot.png", res=150, width=1000, height=1000)
phewasManhattan(results, title="BAG3", annotate.size=3, max.x=1500)
dev.off()